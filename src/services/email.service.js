...
const sendResetPasswordEmail = async (to, token) => {
  ...
  const resetPasswordUrl = `https://event-manager-bknz.onrender.com/v1/auth/reset-password?token=${token}`;
...
const sendVerificationEmail = async (to, token) => {
  ...
  const verificationEmailUrl = `https://event-manager-bknz.onrender.com/v1/auth/verify-email?token=${token}`;
  const text = `Dear user,
